...
void print_yml(const YAML::Node &node, int level) {
    if (node.IsScalar()) {
        LOG_INFO(ROOT_LOGGER,
                 node.Scalar() + " - " + node.Tag() + " - " + std::to_string(level));
    } else if (node.IsNull()) {
        LOG_INFO(ROOT_LOGGER, "NULL - " + node.Tag() + " - " + std::to_string(level));
    } else if (node.IsMap()) {
...
void test_std_type() {
    ...
    auto g_int_unordered_set_val_config = wtsclwq::Config::Lookup(
        "system.int_unordered_set", std::unordered_set<int>{1, 1, 1, 2, 2, 2, 3},
        "a unordered set of int");
    ...
    auto g_complex_map_str_int_config = wtsclwq::Config::Lookup(
        ...
        "a map of string to int");
    auto g_sim_unordered_map_str_int_config = wtsclwq::Config::Lookup(
        "system.ages_sim_unordered_map",
        std::unordered_map<std::string, int>{{"LWQ", 1}, {"SXH", 2}, {"LLB", 3}},
