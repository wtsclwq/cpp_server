 ...
 * @date 2021-09-25
 */
#include "../src/include/http/http_parser.h"

const char test_request_data[] =
    "POST /login?aa=bb#sss HTTP/1.1\r\n"
    "Host: www.sylar.top\r\n"
    "Content-Length: 10\r\n\r\n"
    "1234567890";

const char test_request_chunked_data[] =
...
void test_request(const char *str) {
    wtsclwq::HttpRequestParser parser;
    std::string tmp = str;
    std::cout << "<test_request>:" << std::endl << tmp << std::endl;
    parser.Execute(&tmp[0], tmp.size());
    if (parser.HasError()) {
        std::cout << "parser execute fail" << std::endl;
    } else {
        wtsclwq::HttpRequest::ptr req = parser.GetData();
        std::cout << req->ToString() << std::endl;
    }
}

void test_response(const char *str) {
    wtsclwq::HttpResponseParser parser;
    std::string tmp = str;
    std::cout << "<test_response>:" << std::endl << tmp << std::endl;
    parser.Execute(&tmp[0], tmp.size());
    if (parser.HasError()) {
        std::cout << "parser execue fail" << std::endl;
    } else {
        wtsclwq::HttpResponse::ptr rsp = parser.GetData();
